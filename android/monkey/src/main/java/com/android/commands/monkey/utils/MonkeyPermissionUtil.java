...
import com.android.commands.monkey.events.base.MonkeyPermissionEvent;
...
public class MonkeyPermissionUtil {
        ...
        try {
            ...
            for (Object o : pkgInfos) {
                ...
                for (String perm : info.requestedPermissions) {
                    ...
                    PermissionInfo pi = APIAdapter.getPermissionInfo(mPm, perm, 0);
                    if (pi != null && shouldTargetPermission(info.packageName, pi)) {
                        Logger.infoPrintln(pi.toString());
                        permissions.add(pi);
                    }
                }
            ...
            Logger.warningPrintln("Failed talking with package manager!");
            return false;
        }
    ...
    public void dump() {
        Logger.println("Targeted packages and permissions:");
        for (Map.Entry<String, List<PermissionInfo>> e : mPermissionMap.entrySet()) {
            Logger.println(String.format("  + Using %s", e.getKey()));
            for (PermissionInfo pi : e.getValue()) {
                ...
                Logger.println(String.format("    Permission: %s [%s]", name, group));
            }
        }
    }
