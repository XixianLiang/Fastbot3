...
void Activity::fillValuableWidget(ReuseStatePtr state) {
    ...
    for (auto widget : state->getValuableWidgets()) {
        ...
        auto valuableWidget = std::find_if(
            _valuableWigets.begin(), _valuableWigets.end(),
            [widget](ValuableWidgetPtr predicate) { return widget->getBounds()->hash() == predicate->hash(); });
        ...
        if (valuableWidget == _valuableWigets.end()) {
            ...
            if (it != _widgetMap.end()) {
                BLOG("Activity: find valuableWidget through bound hash failed, but find one through widget's hash");
                it->second->fillDetails(widget);
                _widgetMap[widget->hash()] = it->second;
            }
            // Neither the position hash nor the widget hash can find the corresponding valuableWidget, then create a
            // new one
            else {
                ...
                BLOG("Activity: add a new valuable widget: %s", widget->toString().c_str());
            }

        } else {
            // Add widget information to it
            BLOG("Activity: find valuableWidget through bound's hash");
...
void Activity::addSubSequentActivity(ActivityPtr dst, ActionPtr action) {
    ...
    if (it == _edges.end()) {
        BLOG("Activity: add a brand new edge, from %s to %s", _name.c_str(), dst->_name.c_str());
        _edges.insert(edge);
    } else {
        BLOG("Activity: edge already exist in %s, ignore it", _name.c_str());
    }
}
