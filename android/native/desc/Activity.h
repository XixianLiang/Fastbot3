...
class Activity {
public:
    explicit Activity(ReuseStatePtr state);
     ...
     * First sort the valuableWidgets (according to the screen coordinate top of the component), and generate the
     * valuableWidget description in order from top to bottom.
     *
     * @return std::string
     */
...
public:
    ...
    bool operator<(const ActivityGraphEdge &other) const { return this->_hash < other._hash; }
};
